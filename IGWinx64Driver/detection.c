...
)
{
    ...
    DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL,"Detection: IsPathExcluded called for %wZ.\n", PathName);
...
)
{
    ...
    UNREFERENCED_PARAMETER(pathName);
    DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL,"Detection: ArIsPathMonitored (placeholder) called for %wZ.\n", pathName);
    return TRUE; 
}
...
)
{
    ...
    if (IsPathExcluded(fileName)) {
        DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL,"Detection: Path %wZ is excluded from scanning.\n", fileName);
        return FALSE;
    }

    DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL,"Detection: Scanning buffer for %wZ (Length: %lu)...\n", fileName, length);
        ...
        if (rule->PatternData && rule->PatternLength > 0 && length >= rule->PatternLength) {
            for (ULONG i = 0; i <= length - rule->PatternLength; ++i) {
                if (RtlCompareMemory((PUCHAR)buffer + i, rule->PatternData, rule->PatternLength) == rule->PatternLength) {
                    DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL,"!!! Detection: Rule '%wZ' detected in %wZ !!!\n", &rule->RuleName, fileName);
...
)
{
    ...
    DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL,"Detection: ScanFileContent - Starting full file scan for %wZ.\n", &fileObject->FileName);
    ...
    while ((ULONGLONG)byteOffset.QuadPart < fileSize && !fileDetected) {
        ...
        if (ScanBuffer
            (readBuffer, bytesRead, &fileObject->FileName, process)) {
            fileDetected = TRUE;
            break;
        }
